...
{
    internal class Pm10BusanDividendFactory : IDividendFactory
    {
        ...
        public DividendHistory? CreateDividendHistory(DateOnly dividendDate)
        {
            ...
            var busan = (from item in data.Response.Body.Items
                         where item.DataTime.Equals(dividendDate.ToString("yyyy-MM-dd"), StringComparison.OrdinalIgnoreCase)
                         select item.Busan)
                        .First();

            return DividendHistory.Of(TokenSourceId, int.Parse(busan), dividendDate);
        }
    }
}
